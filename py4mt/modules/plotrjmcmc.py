...
class Results():
    def __init__(
        ...
        self._stationInfoFile = os.path.join(
            self._stationDir, "station_info.txt")
        ...
        # Plot file names
        self._jpgFile = os.path.join(
            self._stationDir,
            "%s_lineardepth.jpg" %
            (self._station))
        self._epsFile = os.path.join(
            self._stationDir,
            "%s_lineardepth.eps" %
            (self._station))
        self._pdfFile = os.path.join(
            self._stationDir,
            "%s_lineardepth.pdf" %
            (self._station))
        ...
        # Title
        self._titleString = (
            "MT Station %s - 1D rj-McMC Inversion" %
            (self._station))
        ...
        # Load bins
        self._depthBins = np.loadtxt(
            os.path.join(
                self._stationDir,
                "depth_bins.txt"))
        self._resBins = np.loadtxt(
            os.path.join(
                self._stationDir,
                "prop_1_bins.txt"))
    ...
    def plot(self):
        ...
        def ShrinkAxes(ax, wfactor=1, hfactor=1):
            ...
            ax.set_position([box.x0 + shiftx, box.y0 + shifty,
                             box.width * wfactor, box.height * hfactor])
        # end func
        ...
        self._ax1 = self._fig.add_axes([0 +
                                        self._hpad, 0.6666 +
                                        self._vpad, 0.5 -
                                        self._hpad, 0.3333 -
                                        self._vpad])  # top left
        self._ax2 = self._fig.add_axes([0 +
                                        self._hpad, 0.3333 +
                                        self._vpad, 0.5 -
                                        self._hpad, 0.3333 -
                                        self._vpad])  # middle left
        self._ax3 = self._fig.add_axes([0 +
                                        self._hpad, 0 +
                                        self._vpad, 0.25 -
                                        self._hpad, 0.3333 -
                                        self._vpad])  # lower-left first
        self._ax4 = self._fig.add_axes([0.25 +
                                        self._hpad, 0 +
                                        self._vpad, 0.25 -
                                        self._hpad, 0.3333 -
                                        self._vpad])  # lower-left second
        self._ax5 = self._fig.add_axes([0.5 +
                                        self._hpad, 0.05 +
                                        self._vpad, 0.3 -
                                        self._hpad, 0.95 -
                                        self._vpad])  # middle
        self._ax5cb = self._fig.add_axes([0.5 +
                                          self._hpad, 0 +
                                          self._vpad, 0.3 -
                                          self._hpad, 0.05 -
                                          self._vpad])  # middle cb
        self._ax6 = self._fig.add_axes([0.8 +
                                        self._hpad *
                                        0.5, 0.05 +
                                        self._vpad, 0.2 -
                                        self._hpad, 0.95 -
        ...
        if (self._plotMisfits):
            ...
            self._ax3.plot([self._burninSampls, self._burninSampls],
                           yl, "--", dashes=(5, 10), color="k", lw=0.2)
            self._ax3.plot(xl, [self._ndata, self._ndata],
                           "--", dashes=(5, 10), color="k", lw=0.2)
...
#        a = np.sort(im)
        ...
        cbinfo = self._ax5.pcolormesh(
            np.power(
                10., self._resBins), np.power(
                10., self._depthBins), im, cmap=plt.get_cmap(
                self._colorMap, 256), norm=colors.LogNorm(
...
#        cbar.ax.set_xticklabels([])
        ...
        self._ax6.plot(
            self._partitionDepthHist,
            np.power(
                10.,
                self._partitionDepth),
            "b",
            lw=1)

        xt = self._ax6.get_xticks()
        xtlabels = []
        for element in xt:
            xtlabels.append(str(math.floor(element/1000))+"k")
        self._ax6.set_xticklabels(xtlabels)
        ...
        for i in np.arange(self._nchains):
            if (i == 0):
                self._ax1.loglog(self._BFFM["period"][i,
                                                      :],
                                 self._BFFM["data1"][i,
                                                     :],
                                 "b-",
                                 lw=1,
                                 zorder=2,
                                 label="Best fit for each chain")
            else:
                self._ax1.loglog(
                    self._BFFM["period"][i, :], self._BFFM["data1"][i, :], "b-", lw=1, zorder=2)
        ...
        for i in np.arange(self._nchains):
            if (i == 0):
                self._ax2.loglog(self._BFFM["period"][i,
                                                      :],
                                 self._BFFM["data2"][i,
                                                     :],
                                 "b-",
                ...
                self._ax2.loglog(
                    self._BFFM["period"][i, :], self._BFFM["data2"][i, :], "b-", lw=1, zorder=2)

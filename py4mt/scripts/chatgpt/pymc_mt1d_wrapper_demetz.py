"""
PyMC DEMetropolisZ wrapper for 1-D anisotropic MT forward (mt1d_aniso),
including EDI/NPZ/HDF5 loaders and a short CLI. This version runs
gradient-free differential-evolution MCMC (DEMetropolisZ) instead of NUTS.

Usage examples
--------------
python pymc_mt1d_wrapper_demetz.py --data site.edi --niter 4000 --threads 4 --plot
python pymc_mt1d_wrapper_demetz.py --data pack.npz --site M01 -j 2 --niter 6000
python pymc_mt1d_wrapper_demetz.py --data db.h5 --group /edi --site ST05 --mask all --niter 8000

Notes
-----
- DEMetropolisZ does not require analytic gradients. The underlying model and
  data loaders are identical to the NUTS version, so you can compare chains
  across samplers for robustness checks.
- You may want higher `--niter` than with NUTS; DEMetropolisZ generally mixes
  slower but is robust to non-smooth posteriors.
...
# Optional h5 support (for HDF5 input)
try:
...
# Forward + helpers from the uploaded aniso.py
...
def _stack_z_components(Z: np.ndarray, comp_mask: np.ndarray) -> np.ndarray:
    """Stack selected components of a 2x2 impedance tensor into a 1D Re/Im vector."""
...
def _stack_sensitivities(
    ...
    """Pack layerwise sensitivity tensors into stacked Re/Im vectors."""
    nl = dZdal.shape[0]
    blocks = []
...
class MT1DImpedanceOp(Op):
    """Forward Op computing impedances; grad provided (unused by DEMetropolisZ)."""
...
class MT1DImpedanceVJP(Op):
    """Vector–Jacobian product using analytic sensitivities (not needed for DEMetropolisZ)."""
...
def _parse_edi_blocks(text: str) -> Dict[str, np.ndarray]:
    """Tolerant EDI parser for the project formats (Phoenix/EGI-like)."""
    txt = text.upper()
...
def load_npz(path: str, site: Optional[str] = None) -> Dict[str, np.ndarray]:
    """Load NPZ exported by the EDI project scripts (flexible keys)."""
...
def load_hdf5(path: str, site: Optional[str] = None, group: str = "/") -> Dict[str, np.ndarray]:
    """Load HDF5 exported by the EDI project scripts (heuristic discovery)."""
...
def build_pymc_model(
    ...
    """Construct a PyMC model to fit complex impedances with diagonal Gaussian noise."""
    ...
    with pm.Model() as model:
        log_al = pm.Normal("log_al", mu=np.log(al0), sigma=2.0, shape=layani.size)
        log_at = pm.Normal("log_at", mu=np.log(at0), sigma=2.0, shape=layani.size)
        log_h = pm.Normal("log_h", mu=np.log(h0), sigma=1.5, shape=layani.size)
        d_blt = pm.Normal("d_blt", mu=0.0, sigma=np.pi / 4, shape=layani.size)
        ...
        sig_flat = pt.as_tensor_variable(sig.reshape(-1))
...
def main():
    """CLI for DEMetropolisZ sampling with EDI/NPZ/HDF5 inputs."""
    parser = argparse.ArgumentParser(description="Run PyMC DEMetropolisZ inversion on anisotropic MT model.")
    ...
    parser.add_argument("--niter", type=int, default=4000, help="Number of posterior draws.")
    parser.add_argument("--tune", type=int, default=None, help="Number of tuning steps (default: niter//2).")
    parser.add_argument("--adapt-interval", type=int, default=100,
                        help="Tuning/adaptation interval for DEMetropolisZ (iterations).")
    parser.add_argument("--threads", "-j", type=int, default=1,
                        help="Number of parallel sampling threads/chains (default: 1).")
    ...
    if sigma_loaded is not None and isinstance(sigma_loaded, np.ndarray):
        if sigma_loaded.ndim == 2 and sigma_loaded.shape[1] == 8:
    ...
    # Default 3-layer starting model
    nl = 3
    ...
    n_chains = max(1, int(args.threads))
    tune = args.tune if args.tune is not None else max(500, args.niter // 2)
    print(
        f"Sampling (DEMetropolisZ) site '{name}' with {args.niter} draws, "
        f"tune={tune}, {n_chains} parallel chain(s)..."
    )

    step = pm.DEMetropolisZ(adapt_interval=int(args.adapt_interval))

    with model:
        idata = pm.sample(
            draws=int(args.niter),
            tune=int(tune),
            chains=n_chains,
            cores=n_chains,
            step=step,
    ...
    if args.plot:
        try:
            ...
            plt.suptitle(f"Posterior (DEMetropolisZ) — {name}")
            plt.show()

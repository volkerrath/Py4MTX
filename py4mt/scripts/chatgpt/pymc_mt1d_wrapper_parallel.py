...
- CLI:
      python pymc_mt1d_wrapper.py --data site.edi --niter 1000 --threads 4 --plot
      python pymc_mt1d_wrapper.py --data site_data.npz --site SITE_A -j 2
      python pymc_mt1d_wrapper.py --data pack.h5 --site SITE_B --group /edi --threads 8
...
def main():
    ...
    tune = max(200, args.niter // 2)
    print(
        f"Sampling site '{name}' with {args.niter} draws (tune={tune}), "
        f"{n_chains} parallel chain(s)..."
    )

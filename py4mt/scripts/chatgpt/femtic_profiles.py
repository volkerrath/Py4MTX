...
def vertical_profile_rbf_local(x0: float, y0: float, z_samples: np.ndarray,
    ...
    M = centroids.shape[0]; P = z_samples.shape[0]
    prof_log = np.empty(P, dtype=float)
    cx, cy, cz = centroids[:,0], centroids[:,1], centroids[:,2]
    for i, z in enumerate(z_samples):
        d = np.sqrt((cx - x0)**2 + (cy - y0)**2 + (cz - z)**2)
        if k >= M: nn = np.arange(M)
        else: nn = np.argpartition(d, k-1)[:k]
        pts = centroids[nn]; vals = v_log[nn]
        D = _pairwise_dists(pts, pts); dq = np.linalg.norm(pts - np.array([x0,y0,z]), axis=1)
        if epsilon is None:
...
def _cli():
    ...
    ap.add_argument("--npz", required=True, help="Path to NPZ with 'centroid' and values.")
    ...
    ap.add_argument("--x", type=float, help="Single profile X if --xy not given")
    ap.add_argument("--y", type=float, help="Single profile Y if --xy not given")
    ...
    coords = [(args.x, args.y)] if not args.xy else [(float(x), float(y)) for x,y in args.xy]
    ...
    for (x, y) in coords:
        ...
        if args.logx and args.out_space == "log10":
            ...
            prof_plot = prof
        profs_for_plot.append(prof_plot); labels.append(label)

    fig, ax = plot_vertical_profiles(z_samples, profs_for_plot, labels=labels,
                                     logx=args.logx, z_positive_down=args.z_positive_down)

    if args.out_png:
        fig.savefig(args.out_png, dpi=200, bbox_inches="tight"); print("Saved PNG:", args.out_png)
    ...
    if len(coords) == 1:
        ...
        if args.out_csv:
            ...
            with open(args.out_csv, "w", newline="") as f:
                w = csv.writer(f); w.writerow(["z", "value_" + args.out_space])
                for z, v in zip(z_samples, prof_exact): w.writerow([z, v])
        ...
        if args.out_hdf5:
            try:
                ...
                with h5py.File(args.out_hdf5, "w") as h5:
                    ...
                    h5.attrs["method"] = args.method; h5.attrs["kernel"] = args.kernel if args.method == "rbf" else ""

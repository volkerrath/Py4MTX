...
def _plot_plan_view(points_xy, centroids_xy, out_png=None):
    ...
    if out_png:
        ...
        print('Saved plan-view PNG:', out_png)
    return fig, ax
...
def aggregate_per_region(region: np.ndarray, rho_log: np.ndarray,
    ...
    for i, rid in enumerate(reg_ids):
        m = (region == rid)
        vals = rho_log[m]
        v_log[i] = float(np.mean(vals)) if method == "mean_log10" else float(
            np.median(vals))
        lo_agg[i] = float(np.median(lo_log[m])) if (
            lo_log is not None and m.any()) else np.nan
        hi_agg[i] = float(np.median(hi_log[m])) if (
            hi_log is not None and m.any()) else np.nan
...
def run_pipeline(mesh_path: str, rho_path: Optional[str],
    ...
    raise NotImplementedError(
        "run_pipeline depends on project-specific mesh/array readers not included here.")


def _cli():
    import argparse
    ap = argparse.ArgumentParser(
        description="(Skeleton) FEMTIC pipeline: arrays -> edits -> aggregate -> inject rho (+ optional slice).")
    ap.add_argument("--mesh", required=False,
                    help="Path to mesh.dat (reader not included here)")
    ap.add_argument("--rho", default=None,
                    help="Path to resistivity_block_iterX.dat")
    ap.add_argument("--template-rho", required=True,
                    help="Template rho-file to inject into")
    ap.add_argument("--out-rho", required=True,
                    help="Output rho-file path (new name)")
    ...
    ap.add_argument(
        "--agg", choices=["median_log10", "mean_log10"], default="median_log10")
    ap.add_argument("--write-space",
                    choices=["linear", "log10"], default="linear")
    ...
    ap.add_argument("--n", dest="override_n", type=int,

...
def rho_phase_from_Z(Z: np.ndarray, f: float):
    ...
    phi = np.degrees(np.array([np.arctan2(z.imag, z.real)
                     for z in comps], dtype=float))
    return rho, phi
...
def run(in_edi: Path, out_base: Path, write_csv: bool, write_hdf5: bool,
    ...
    if prefer_spectra and spectra_blocks:
        ...
        h5_payload = ('spectra', station, freqs, Z_all,
                      T_all, rho_all, phi_all, PT_all)
    else:
        ...
        h5_payload = ('tables', station, freqs, Z_all,
                      T_all, rho_all, phi_all, PT_all)

    df = pd.DataFrame(rows).sort_values(
        'freq_Hz', ascending=False).reset_index(drop=True)
    ...
    if write_csv:
        header = [
            ...
            f"# Station: {station} | Rotate: {
                rotate_deg:.3f} deg | Prefer SPECTRA: {prefer_spectra} | Ref: {ref}",
    ...
    if long:
        ...
        for _, r in df.iterrows():
            ...
            for comp in ['zxx', 'zxy', 'zyx', 'zyy', 'tx', 'ty', 'ptxx', 'ptxy', 'ptyx', 'ptyy']:
                recs.append({'freq_Hz': f, 'component': comp +
                            '_re', 'value': r[comp+'_re']})
                recs.append({'freq_Hz': f, 'component': comp +
                            '_im', 'value': r[comp+'_im']})
            for comp in ['rho_xx', 'rho_xy', 'rho_yx', 'rho_yy', 'phi_xx_deg', 'phi_xy_deg', 'phi_yx_deg', 'phi_yy_deg']:
                recs.append(
                    {'freq_Hz': f, 'component': comp, 'value': r[comp]})
        dfl = pd.DataFrame(recs).sort_values(['freq_Hz', 'component'], ascending=[
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Convert EDI (standard or Phoenix spectra) to CSV/HDF5 with Z, T, ρ/φ, Phase Tensor.")
    ap.add_argument("edi", type=Path, help="Path to input EDI file")
    ap.add_argument("--out-base", type=Path, default=None,
                    help="Output base path (no suffix). Defaults to <station>")
    ...
    ap.add_argument("--rotate-deg", type=float, default=0.0,
                    help="Rotate E/H frame by θ degrees (CCW)")
    ap.add_argument("--fill-invalid", type=float, default=None,
                    help="Fill non-finite entries with this constant (e.g., 1e-30)")
    ap.add_argument("--prefer-spectra", action="store_true",
                    help="If SPECTRA present, prefer spectra-reconstructed Z/T")
    ap.add_argument("--ref", choices=["H", "RH"], default="RH",
                    help="H reference for SPECTRA path (default RH)")
    ap.add_argument("--long", action="store_true",
                    help="Also write a tidy long-format CSV with a 'component' column")
    args = ap.parse_args(argv)

    edi_text = args.edi.read_text(encoding='latin-1', errors='ignore')
    m = re.search(
        r'DATAID\s*=\s*"?([A-Za-z0-9_\-\.]+)"?', edi_text, flags=re.IGNORECASE)
    station = m.group(1) if m else 'UNKNOWN'
    out_base = args.out_base if args.out_base is not None else args.edi.with_name(
        station)

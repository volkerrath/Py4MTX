...
def _kernel(r: np.ndarray, kernel: str, epsilon: float) -> np.ndarray:
    ...
    if kernel == "thin_plate":
        out = np.zeros_like(r); m = r > 0; rm = r[m]; out[m] = (rm*rm) * np.log(rm); return out
...
def _rbf_local(q: np.ndarray, pts: np.ndarray, vals: np.ndarray,
    ...
    nn = np.arange(pts.shape[0]) if k >= pts.shape[0] else np.argpartition(d, k-1)[:k]
    P = pts[nn]; v = vals[nn]
    D = _pairwise_dists(P, P); dq = np.linalg.norm(P - q, axis=1)
    if epsilon is None:
        md = np.median(D[D>0]) if np.any(D>0) else np.median(d[nn]); eps = 1.0 / max(md, 1e-9)
    else: eps = float(epsilon)
    A = _kernel(D, kernel=kernel, epsilon=eps); A.flat[::A.shape[0]+1] += smooth
    b = v
...
def _idw(q: np.ndarray, pts: np.ndarray, vals: np.ndarray, power: float, eps: float=1e-6) -> float:
    d2 = np.sum((pts - q)**2, axis=1) + eps**2; w = 1.0 / (d2 ** (power/2.0))
    return float(np.sum(w * vals) / np.sum(w))

def build_polyline(points_xy: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    diffs = np.diff(points_xy, axis=0); seglen = np.sqrt(np.sum(diffs**2, axis=1))
    s = np.concatenate([[0.0], np.cumsum(seglen)]); dirs = (diffs / np.maximum(seglen[:,None], 1e-12))
    return s, dirs

def sample_polyline(points_xy: np.ndarray, ns: int) -> Tuple[np.ndarray, np.ndarray]:
    s, _ = build_polyline(points_xy); total = s[-1]; S = np.linspace(0.0, total, ns)
    XY = np.empty((ns, 2), dtype=float); seg_s = s
    for i, si in enumerate(S):
        j = np.searchsorted(seg_s, si, side="right") - 1; j = max(0, min(j, len(seg_s)-2))
...
def curtain_slice(points_xy: np.ndarray, z_samples: np.ndarray,
    ...
    if centroids.shape[1] >= 6:
        m = (centroids[:,5] != 1); centroids = centroids[m]; values = values[m]
    v_log = _from_linear(values, space=in_space)
    nz = z_samples.size; S, XY = sample_polyline(points_xy, ns)
    ...
    for j in range(ns):
        ...
        for i, z in enumerate(z_samples):
            q = np.array([x, y, z])
            if method == "rbf":
                V_log[i, j] = _rbf_local(q, centroids, v_log, k, kernel, epsilon, smooth)
            else:
...
def plot_curtain_pyvista(S: np.ndarray, Z: np.ndarray, V: np.ndarray, XYZ_curve: np.ndarray,
    ...
    try:
        ...
        raise ImportError("PyVista is required for plot_curtain_pyvista(). Install pyvista.") from e
    ns = S.size; nz = Z.size
    ...
    if logscale:
        ...
        grid["log10_resistivity"] = scalars; scal_name = "log10_resistivity"
    else:
        scal_name = "resistivity"
    sargs = dict(title="log10(ρ)" if logscale else "ρ (Ω·m)")
    p.add_mesh(grid, scalars=scal_name, cmap=cmap, clim=clim, scalar_bar_args=sargs)
    p.add_axes(); p.add_text(title, font_size=12); p.show_grid()
    return p, grid

def _cli():
    import argparse, numpy as _np
    ap = argparse.ArgumentParser(description="Curtain slice along XY polyline (RBF/IDW) with PyVista plotting.")
    ap.add_argument("--npz", required=True, help="NPZ with 'centroid' and values.")
    ap.add_argument("--values-key", default="log10_resistivity", help="Key for values in NPZ (default: log10_resistivity)")
    ap.add_argument("--polyline-csv", help="CSV with columns x,y listing polyline vertices (in order).")
    ...
    ap.add_argument("--zmin", type=float, required=True); ap.add_argument("--zmax", type=float, required=True)
    ap.add_argument("--nz",  type=int,   default=201); ap.add_argument("--ns",  type=int,   default=301)
    ap.add_argument("--method", choices=["rbf","idw"], default="rbf")
    ap.add_argument("--k", type=int, default=50)
    ap.add_argument("--kernel", choices=["gaussian","multiquadric","inverse_multiquadric","linear","thin_plate"], default="gaussian")
    ap.add_argument("--epsilon", type=float, default=None); ap.add_argument("--smooth", type=float, default=1e-6)
    ...
    ap.add_argument("--screenshot", default=None, help="Save PyVista screenshot PNG")
    ap.add_argument("--no-show", action="store_true", help="Do not show interactive window")
    args = ap.parse_args()

    data = _np.load(args.npz)
    if "centroid" not in data: raise KeyError("NPZ must contain 'centroid' (M,3) array.")
    centroids = data["centroid"]
    if "flag" in data:
        m = (data["flag"] != 1); centroids = centroids[m]
    if args.values_key in data:
        values = data[args.values_key]; in_space = args.in_space
    else:
        if "log10_resistivity" in data: values = data["log10_resistivity"]; in_space = "log10"
        elif "resistivity" in data: values = data["resistivity"]; in_space = "linear"
        else: raise KeyError("Could not find values in NPZ.")
    if "flag" in data: values = values[m]
    ...
    if args.polyline_csv:
        ...
        with open(args.polyline_csv, "r") as f:
            ...
            for row in r:
                if not row: continue
                try:
                    x = float(row[0]); y = float(row[1]); pts.append([x,y])
                except Exception: continue
        points_xy = _np.asarray(pts, dtype=float)
    else:
        if not args.xy or len(args.xy) < 2:
            raise ValueError("Provide at least two vertices via --xy or use --polyline-csv.")
        points_xy = _np.asarray([[float(x), float(y)] for x,y in args.xy], dtype=float)
    ...
    try:
        ...
        if args.vtk: 
            try: grid.save(args.vtk); print("Saved VTK:", args.vtk)
            except Exception as e: print("Failed to save VTK:", e)
        if args.screenshot:
            try: p.screenshot(args.screenshot); print("Saved screenshot:", args.screenshot)
            except Exception as e: print("Failed to save screenshot:", e)
        if not args.no_show: p.show()
    except ImportError as e:
        print("PyVista not available; skipping interactive plot.", e)

    if args.npz_out:
        _np.savez_compressed(args.npz_out, S=S, Z=Z, V=V, polyline=points_xy); print("Saved NPZ:", args.npz_out)
    if args.csv_out:
        import csv
        with open(args.csv_out, "w", newline="") as f:
            w = csv.writer(f); w.writerow(["s","z","value_"+args.out_space])
            for j, s in enumerate(S):
                for i, z in enumerate(Z): w.writerow([s, z, V[i,j]])

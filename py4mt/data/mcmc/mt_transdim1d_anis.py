...
RHO_MAX_ANISO = 10000.*np_ones_like(H_M)   # np.array([200.0, 300.0, 500.0, 800.0, 300.0, 100.0])
#  rho_min (minimum horizontal resistivity, Ohm·m)
#  Ratio ≈ 1 for isotropic layers; > 1 for anisotropic layers.
RHO_MIN_ANISO = 1.*np_ones_like(H_M) #np.array([200.0, 100.0, 100.0, 200.0, 300.0, 100.0])
#  strike angles (degrees)
STRIKE_ANISO = 0.*np_ones_like(H_M) #np.array([0.0, 30.0, 45.0, 45.0, 0.0, 0.0])
...
N_CHAINS = 20
N_JOBS = 20       # -1 = all CPUs; 1 = sequential (debug)
